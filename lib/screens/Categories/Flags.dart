...
class _FlagsState extends State<Flags> with SingleTickerProviderStateMixin {
    ...
    _timer = Timer.periodic(const Duration(seconds: 1), (timer) {
      if (_timeLeft > 0) {
        ...
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (context) => ScoreScreen(tete: 'Drapeaux',score: score, totalQuestions: totalQuestions),
          ),
        );
      }
    });
  ...
  void generateQuestion() {
    ...
    question = 'Choisissez le nom correspondant à ce drapeau :';
    correctAnswer = currentCountry['name'];

    options = [correctAnswer];
    while (options.length < 4) {
      String randomCountry = countries[random.nextInt(countries.length)]['name'];
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: mediumTextBlack('Drapeaux'),
