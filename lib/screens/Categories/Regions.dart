...
class _RegionsState extends State<Regions> with SingleTickerProviderStateMixin {
    ...
    _timer = Timer.periodic(const Duration(seconds: 1), (timer) {
      if (_timeLeft > 0) {
        ...
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (context) => ScoreScreen(tete: 'Régions',score: score, totalQuestions: totalQuestions),
          ),
        );
      }
    });
  ...
  void generateQuestion() {
    ...
    while (options.length < 4) {
      String randomRegion = countries[random.nextInt(countries.length)]['region'];
      if (!options.contains(randomRegion)) {
        options.add(randomRegion);
      }
    }
    options.shuffle();
