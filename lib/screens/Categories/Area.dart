...
class _AreaState extends State<Area> with SingleTickerProviderStateMixin {
    ...
    _timer = Timer.periodic(const Duration(seconds: 1), (timer) {
      if (_timeLeft > 0) {
        ...
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (context) => ScoreScreen(tete: 'Superficie',score: score, totalQuestions: totalQuestions),
          ),
        );
      }
    });
  ...
  Future<void> loadCountries() async {
    ...
    // On filtre uniquement les pays avec une superficie valide
    countries = countries.where((c) => c['area'] != null && c['area'] > 0).toList();
  ...
  void generateQuestion() {
    ...
    correctAnswer = area.toStringAsFixed(0); // arrondi à l'entier

    options = [correctAnswer];

    while (options.length < 4) {
      double randomArea = (countries[random.nextInt(countries.length)]['area'] as num).toDouble();
      String areaStr = randomArea.toStringAsFixed(0);
      if (!options.contains(areaStr)) {
        options.add(areaStr);
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: mediumTextBlack('Superficie'),
