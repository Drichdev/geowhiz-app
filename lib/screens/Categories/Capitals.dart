...
class Capitals extends StatefulWidget {
  const Capitals({super.key});

  @override
  _CapitalsState createState() => _CapitalsState();
}

class _CapitalsState extends State<Capitals> with SingleTickerProviderStateMixin {
    ...
    _timer = Timer.periodic(const Duration(seconds: 1), (timer) {
      if (_timeLeft > 0) {
        ...
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (context) => ScoreScreen(tete: 'Capital',score: score, totalQuestions: totalQuestions),
          ),
        );
      }
    });
  ...
  Future<void> loadCountries() async {
    ...
    // On filtre uniquement les pays avec une capitale connue
    countries = countries.where((c) => c['capital'] != null && c['capital'].toString().isNotEmpty).toList();
  ...
  void generateQuestion() {
    ...
    while (options.length < 4) {
      String randomCapital = countries[random.nextInt(countries.length)]['capital'];
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: mediumTextBlack('Capitales'),
